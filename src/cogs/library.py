...
class Library(discord.Cog):
  ...
  library_group = SlashCommandGroup("library",
                                    "Commands to do with our server library")
  ...
  @discord.option("author",
                  description="The name/nickname of the work's author")
  ...
  async def submit(self, ctx, title: str, author: str, genre: str, url: str):
    if isinstance(ctx.channel, discord.channel.DMChannel):
      return await ctx.send_response(
        "This command cannot be run in DMs. If you have permission, run this command in the Writer's Cave server instead."
      )
  ...
  async def approve(self, ctx, id: int):
    ...
    try:
      ...
      return await response.edit_original_response(
        content="That ID isn't valid.")

    await response.edit_original_response(
      content="Approved submission. Notifying the author...")

    user = await self.bot.fetch_user(sub["submitted_by"])
    await user.send(
      f"Congratulations {user.mention}, your submission \"{sub['title']}\" was approved! It'll appear in our library soon."
    )
